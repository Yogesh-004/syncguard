   ...
   {!resolved ? (
    <>
     ...
     <div style={{display:'flex', gap:6, marginTop:8}}>
      <input placeholder={`New value for ${conflict.field||'field'}...`} value={editVal} onChange={e=>setEditVal(e.target.value)} style={{flex:1, background:'#0f172a', border:'1px solid #334155', color:'white', padding:'6px 10px', borderRadius: 0, fontSize:12}} />
     ...
     <input placeholder="Reason (optional)..." value={reason} onChange={e=>setReason(e.target.value)} style={{width:'100%', background:'#0f172a', border:'1px solid #334155', color:'white', padding:'6px 10px', borderRadius: 0, fontSize:12, marginTop:8}} />
    ...
    <div style={{fontSize:12, marginTop:6}}>
     ...
     {latest && ['approved','modified'].includes(latest.status) && latest.resolved_value!==null && latest.resolved_value!==undefined && (
      <div style={{display:'flex', gap:6, marginTop:8}}>
       <button disabled={busy} onClick={doDry} style={btn('#b45309')}>Run Dry Run</button>
       <button disabled={busy || !dry} onClick={doPush} style={btn('#22c55e')}>Confirm Push</button>
      </div>
     )}
     {dry && (
      <div style={{background:'#0f172a', borderRadius: 0, padding:8, marginTop:8, fontSize:11}}>
...
function btn(color:string){
 return { background: color, color:'white', border:'none', padding:'6px 10px', borderRadius: 0, fontWeight:700, fontSize:12, cursor:'pointer' } as any
}
